    ...
    */
    ...
    override func drawRect(rect: CGRect) {
        ...
        let boxRect = CGRect ( x: round((bounds.size.width - boxWidth) / 2),
            y :round((bounds.size.height - boxHeight) / 2), width: boxWidth,
            height: boxHeight)
        let roundedRect = UIBezierPath(roundedRect: boxRect, cornerRadius: 10); UIColor(white: 0.3, alpha: 0.8).setFill()
        roundedRect.fill()
